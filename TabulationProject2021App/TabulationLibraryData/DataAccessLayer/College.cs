...
{
    public class College : CandidateModel, ICandidate
    {
        ...
        public void CriteriaGradesEveningGown(string candidateID, string judgeNumber, decimal[] points)
        {
            using (OleDbConnection cn = new OleDbConnection(HelperClass.ConnectionString))
            {
                using (OleDbCommand command = new OleDbCommand())
                {
                    ...
                    command.Parameters.AddWithValue("@SchoolLevel", "College");
        ...
        public bool GetData(int candidateNumber)
        {
            using(OleDbConnection cn = new OleDbConnection(HelperClass.ConnectionString))
            {
                using (OleDbCommand command = new OleDbCommand())
                {
                    ...
                    command.Connection = cn;
                    command.CommandType = System.Data.CommandType.Text;
                    command.CommandText = "SELECT * FROM [CandidatesCollege] WHERE [CandidateID] = @CnNumber ORDER BY [CandidateID] ASC";
        ...
        public void Vote(VoteModel vote, string Event)
        {
            using(OleDbConnection cn = new OleDbConnection(HelperClass.ConnectionString))
            {
                using(OleDbCommand command = new OleDbCommand())
                {
                    ...
                    command.Connection = cn;
                    command.CommandType = System.Data.CommandType.Text;
                    command.CommandText = $@"UPDATE [Scoring{Event}-College] SET [{vote.JudgeNumber}] = @JudgeNumber
